...
def get_wd_w_buffer(start_date, end_date, engine):
    ...
    try:
        new_data = pd.read_sql_query(f"SELECT * FROM sensor_water_depth WHERE date >= '{new_start_date}' AND date <= '{end_date}' AND \"sensor_ID\"!='DE_04'", engine).sort_values(['place','date']).drop_duplicates()
    except:
